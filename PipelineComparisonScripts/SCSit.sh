...
scsit -r1 ${forward} -r2 ${reverse} -p /media/draco/lucask/AlgorithmTest/SCSit/primerList.txt -b /media/draco/lucask/AlgorithmTest/SCSit/barcodeList.txt -t 8 -o ${output}
...
echo -e "ALIGN: $(date)\n" >> ${timefile}
STAR --runThreadN 8 \
     --readFilesIn ${output}/output.R1.fastq \
...
/home/lucask/miniconda3/bin/umi_tools count --wide-format-cell-counts --per-gene --gene-tag=XT --assigned-status-tag=XS --per-cell -I assigned_sorted.bam -S counts.tsv.gz --umi-separator=":"
